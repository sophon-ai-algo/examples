...
public:
  ...
  ~SSD();
  void preForward(std::vector<cv::Mat> &input);
  void forward();
  void postForward(std::vector<cv::Mat> &input, std::vector<std::vector<ObjRect>> &detections);
  void enableProfile(TimeStamp *ts);

private:
  void preprocess_opencv (std::vector<cv::Mat> &input);
  ...
  // opencv objects for storing intermediate results
