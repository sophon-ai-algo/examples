...
Resnet::Resnet(bm::BMNNContextPtr bmctx):m_bmctx(bmctx) {
    m_bmnet = std::make_shared<bm::BMNNNetwork>(m_bmctx->bmrt(), "caffe_resnet50_bmnetc");
...
int Resnet::preprocess(std::vector<bm::ResizeFrameInfo> &frames)
{
    int ret = 0;
...
int Resnet::forward(std::vector<bm::ResizeFrameInfo> &frame_infos)
{
    int ret = 0;
...
int Resnet::postprocess(std::vector<bm::ResizeFrameInfo> &frameinfos)
{
...
void Resnet::extract_feature_cpu(bm::ResizeFrameInfo &frame_info) {

    int frameNum = frame_info.v_resized_imgs.size();
    for(int frameIdx = 0; frameIdx < frameNum;++frameIdx) {
        bm::BMNNTensorPtr output_tensor = get_output_tensor("fc1000", frame_info, 1.0);
...
bm::BMNNTensorPtr Resnet::get_output_tensor(const std::string &name, bm::ResizeFrameInfo& frame_info, float scale=1.0) {
