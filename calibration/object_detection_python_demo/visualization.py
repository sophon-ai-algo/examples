...
def draw_bbox(img, bbox, shape, label, color=[255, 0, 0], thickness=2):
    ...
    cv2.rectangle(img, p1[::-1], p2[::-1], color, thickness)
    p1 = (p1[0] + 15, p1[1])
    cv2.putText(img, str(label), p1[::-1], cv2.FONT_HERSHEY_DUPLEX, 0.5, color,
                1)
...
def bboxes_draw_on_img(img, classes, scores, bboxes, colors, thickness=2):
    ...
    for i in range(bboxes.shape[0]):
        ...
        cv2.rectangle(img, p1[::-1], p2[::-1], color, thickness)
        # Draw text...
        s = '%s/%.3f' % (classes_name[classes[i]], scores[i])
        (text_width, text_height) = cv2.getTextSize(s, font, fontScale=font_scale, thickness=1)[0]
        box_coords = ((p1[0] - 1, p1[1] - 1), (p1[0] - text_height - 7, p1[1] + text_width))
        cv2.rectangle(img, box_coords[0][::-1], box_coords[1][::-1], color, cv2.FILLED)
        p1 = (p1[0] - 5, p1[1])
        cv2.putText(img, s, p1[::-1], font, font_scale, (255, 255, 255), 1)
